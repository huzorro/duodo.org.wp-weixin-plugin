 ...
 * Time: 下午5:29
 */
require_once(plugin_dir_path( __FILE__ ) . 'includes/class-wechat-reply-content-list-table.php');

class Wechat_reply_content_main {
     /**
    ...
    public function add_plugin_admin_menu() {
        ...
        add_submenu_page(
            ...
            __('Reply-content', $this->plugin_slug),
            __('Reply-content', $this->plugin_slug),
            'activate_plugins',
            'reply_content',
            array(&$this, 'wechat_reply_content_page_handler')
        );
        add_submenu_page(
            $this->plugin_slug . "-" . 'settings',
            __('Add Reply-content', $this->plugin_slug),
            __('Add Reply-content', $this->plugin_slug),
            'activate_plugins',
            'reply_content_form',
            array(&$this, 'wechat_reply_content_form_page_handler')
        );
    }
    /**
     * List page handler
     ...
     */
    function wechat_reply_content_page_handler()
    {
        global $wpdb;
        ...
        <div class="wrap">
            ...
            <h2><?php _e('reply content', $this->plugin_slug)?> <a class="add-new-h2"
                                                                 href="<?php echo get_admin_url(get_current_blog_id(), 'admin.php?page=reply_content_form');?>"><?php _e('Add new', $this->plugin_slug)?></a>
            </h2>
            <?php echo $message; ?>
            <?php $table->views();?>
            <form id="persons-table" method="GET">
                ...
                <?php
                $table->search_box(__('search', $this->plugin_slug), 'reply_content_search');
                $table->display(); ?>
            </form>
     ...
     */
    function wechat_reply_content_form_page_handler()
    {
        global $wpdb;
        ...
        if (wp_verify_nonce($_REQUEST['nonce'], basename(__FILE__))) {
            ...
            $item_valid = self::wechat_reply_content_validate($item);
            if ($item_valid === true) {
        ...
        add_meta_box('wechat_reply_content_form_meta_box', __('latest news data', $this->plugin_slug), array(&$this, 'wechat_reply_content_form_meta_box_handler'), 'content', 'normal', 'default');
        ?>
        <div class="wrap">
            <div class="icon32 icon32-posts-post" id="icon-edit"><br></div>
            <h2><?php _e('reply content', $this->plugin_slug)?>
                <a class="add-new-h2" href="<?php echo get_admin_url(get_current_blog_id(), 'admin.php?page=reply_content');?>"><?php _e('back to list', $this->plugin_slug)?></a>
            </h2>
                ...
                <div class="metabox-holder" id="poststuff">
                    <div id="post-body">
                        <div id="post-body-content">
                            ...
                            <?php do_meta_boxes('content', 'normal', $item); ?>
     ...
     * @param $item
     */
    function wechat_reply_content_form_meta_box_handler($item)
    {
//        $selected = isset( $item['status'] ) ? esc_attr( $item['status'] ) : "";
        ...
        <table cellspacing="2" cellpadding="5" style="width: 100%;" class="form-table">
            ...
            <tr class="form-field">
                <th valign="top" scope="row">
                    <label for="reply_packet"><?php _e('reply_packet', $this->plugin_slug)?></label>
                </th>
                <td>
<!--                    <input id="reply_content" name="reply_content" type="text" style="width: 95%" value="<?php /*echo esc_attr($item['reply_content'])*/?>"
                           size="50" class="code" placeholder="<?php /*_e('reply content', $this->plugin_slug)*/?>" required>-->
                    <textarea name="reply_packet" type="text" id="reply_packet"
                              cols="50" rows="6" value="<?php echo esc_attr($item['reply_packet'])?>" required><?php echo esc_attr($item['reply_packet'])?></textarea>
                </td>
            </tr>
            <tr class="form-field">
                <th valign="top" scope="row">
                    <label for="reply_type"><?php _e('reply_type', $this->plugin_slug)?></label>
                </th>
                <td>

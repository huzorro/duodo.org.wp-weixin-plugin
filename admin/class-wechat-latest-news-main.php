...
class Wechat_latest_news_main {
     ...
     */
    function wechat_latest_news_page_handler()
    {
        global $wpdb;

        $table = new Wechat_latest_news_list_table();
        $table->prepare_items();
        ...
        <div class="wrap">
            ...
            <form id="persons-table" method="GET">
                ...
                <?php
                $table->search_box(__('search', $this->plugin_slug), 'latest_news_search');
                $table->display(); ?>
            </form>
     ...
     */
    function wechat_latest_news_form_page_handler()
    {
        global $wpdb;
        $table_name = $wpdb->prefix . 'wechat_latest_news'; // do not forget about tables prefix
        ...
        if (wp_verify_nonce($_REQUEST['nonce'], basename(__FILE__))) {
            ...
            $item_valid = self::wechat_latest_news_validate($item);
            if ($item_valid === true) {
        ...
        add_meta_box('wechat_latest_news_form_meta_box', __('latest news data', $this->plugin_slug), array(&$this, 'wechat_latest_news_form_meta_box_handler'), 'news', 'normal', 'default');
        ?>
        <div class="wrap">
            <div class="icon32 icon32-posts-post" id="icon-edit"><br></div>
            <h2><?php _e('latest news', $this->plugin_slug)?>
                <a class="add-new-h2" href="<?php echo get_admin_url(get_current_blog_id(), 'admin.php?page=latest_news');?>"><?php _e('back to list', $this->plugin_slug)?></a>
            </h2>
                ...
                <div class="metabox-holder" id="poststuff">
                    <div id="post-body">
                        <div id="post-body-content">
                            ...
                            <?php do_meta_boxes('news', 'normal', $item); ?>
     ...
     * @param $item
     */
    function wechat_latest_news_form_meta_box_handler($item)
    {
//        $selected = isset( $item['status'] ) ? esc_attr( $item['status'] ) : "";
        $check = isset( $item['news_type'] ) ? esc_attr( $item['news_type'] ) : '';
        ...
        <table cellspacing="2" cellpadding="5" style="width: 100%;" class="form-table">
            ...
            <tr class="form-field">
                <th valign="top" scope="row">
                    <label for="news_keyword"><?php _e('news_keyword', $this->plugin_slug)?></label>
                </th>
                <td>
                    <input id="news_keyword" name="news_keyword" type="text" style="width: 95%" value="<?php echo esc_attr($item['news_keyword'])?>"
                           size="50" class="code" placeholder="<?php _e('news keyword', $this->plugin_slug)?>" required>
                </td>
            </tr>
            <tr class="form-field">
                <th valign="top" scope="row">
                    <label for="news_packet"><?php _e('news_packet', $this->plugin_slug)?></label>
                </th>
                <td>
<!--                    <input id="reply_content" name="reply_content" type="text" style="width: 95%" value="<?php /*echo esc_attr($item['reply_content'])*/?>"
                    ...
                    <textarea name="news_packet" type="text" id="news_packet"
                              cols="50" rows="6" value="<?php echo esc_attr($item['news_packet'])?>" required><?php echo esc_attr($item['news_packet'])?></textarea>
                </td>
            </tr>
            <tr class="form-field">
                <th valign="top" scope="row">

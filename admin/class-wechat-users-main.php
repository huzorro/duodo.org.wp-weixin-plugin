 ...
 * Time: 下午5:29
 */
require_once(plugin_dir_path( __FILE__ ) . 'includes/class-wechat-users-list-table.php');

class Wechat_users_main {
     /**
    ...
    public function add_plugin_admin_menu() {
        ...
        add_submenu_page(
            ...
            __('View-users', $this->plugin_slug),
            __('View-users', $this->plugin_slug),
            'activate_plugins',
            'users',
            array(&$this, 'wechat_users_page_handler')
        );
        add_submenu_page(
            $this->plugin_slug . "-" . 'settings',
            __('Add-user ', $this->plugin_slug),
            __('Add-user ', $this->plugin_slug),
            'activate_plugins',
            'users_form',
            array(&$this, 'wechat_users_form_page_handler')
        );
    }
    /**
     * List page handler
     ...
     */
    function wechat_users_page_handler()
    {
        global $wpdb;
        ...
        <div class="wrap">
            ...
            <h2><?php _e('users', $this->plugin_slug)?> <a class="add-new-h2"
                                                                 href="<?php echo get_admin_url(get_current_blog_id(), 'admin.php?page=users_form');?>"><?php _e('Add new', $this->plugin_slug)?></a>
            </h2>
            <?php echo $message; ?>
            <?php $table->views();?>
            <form id="persons-table" method="GET">
                ...
                <?php
                $table->search_box(__('search', $this->plugin_slug), 'user_search');
                $table->display(); ?>
            </form>
     ...
     */
    function wechat_users_form_page_handler()
    {
        global $wpdb;
        ...
        if (wp_verify_nonce($_REQUEST['nonce'], basename(__FILE__))) {
            ...
            $item_valid = self::wechat_users_validate($item);
            if ($item_valid === true) {
        ...
        <div class="wrap">
            <div class="icon32 icon32-posts-post" id="icon-edit"><br></div>
            <h2><?php _e('users', $this->plugin_slug)?>
                <a class="add-new-h2" href="<?php echo get_admin_url(get_current_blog_id(), 'admin.php?page=users');?>"><?php _e('back to list', $this->plugin_slug)?></a>
            </h2>
                ...
                <div class="metabox-holder" id="poststuff">
                    <div id="post-body">
                        <div id="post-body-content">
                            ...
                            <?php do_meta_boxes('user', 'normal', $item); ?>
     ...
     * @param $item
     */
    function wechat_users_form_meta_box_handler($item)
    {
        $selected = isset( $item['status'] ) ? esc_attr( $item['status'] ) : "";
        $check = isset( $item['gender'] ) ? esc_attr( $item['gender'] ) : '';
        ?>
        ...
        <table cellspacing="2" cellpadding="5" style="width: 100%;" class="form-table">
            ...
            <tr class="form-field">
                <th valign="top" scope="row">
                    <label for="openid"><?php _e('openid', $this->plugin_slug)?></label>
                </th>
                <td>
                    <input id="openid" name="openid" type="text" style="width: 95%" value="<?php echo esc_attr($item['openid'])?>"
                           size="50" class="code" placeholder="<?php _e('openid', $this->plugin_slug)?>" required>
                </td>
            </tr>
            <tr class="form-field">
                <th valign="top" scope="row">
            ...
            <tr class="form-field">
                ...
                <td>
                    <input id="city" name="city" type="text" style="width: 95%" value="<?php echo esc_attr($item['city'])?>"
                           size="50" class="code" placeholder="<?php _e('city', $this->plugin_slug)?>" required>
                </td>
            </tr>
            <tr class="form-field">
                <th valign="top" scope="row">
                    <label for="country"><?php _e('country', $this->plugin_slug)?></label>
                </th>
                <td>
                    <input id="country" name="country" type="text" style="width: 95%" value="<?php echo esc_attr($item['country'])?>"
                           size="50" class="code" placeholder="<?php _e('country', $this->plugin_slug)?>" required>
                </td>
            </tr>
            <tr class="form-field">
                <th valign="top" scope="row">
                    <label for="province"><?php _e('province', $this->plugin_slug)?></label>
                </th>
                <td>
                    <input id="province" name="province" type="text" style="width: 95%" value="<?php echo esc_attr($item['province'])?>"
                           size="50" class="code" placeholder="<?php _e('province', $this->plugin_slug)?>" required>
                </td>
            </tr>
            <tr class="form-field">
                <th valign="top" scope="row">
                    <label for="language"><?php _e('language', $this->plugin_slug)?></label>
                </th>
                <td>
                    <input id="language" name="language" type="text" style="width: 95%" value="<?php echo esc_attr($item['language'])?>"
                           size="50" class="code" placeholder="<?php _e('language', $this->plugin_slug)?>" required>
                </td>
            </tr>
            <tr class="form-field">
                <th valign="top" scope="row">
                    <label for="headimgurl"><?php _e('headimgurl', $this->plugin_slug)?></label>
                </th>
                <td>
                    <input id="headimgurl" name="headimgurl" type="text" style="width: 95%" value="<?php echo esc_attr($item['headimgurl'])?>"
                           size="50" class="code" placeholder="<?php _e('headimgurl', $this->plugin_slug)?>" required>
                </td>
            </tr>
            <tr class="form-field">
                <th valign="top" scope="row">
                    <label for="user_packet"><?php _e('user_packet', $this->plugin_slug)?></label>
                </th>
                <td>
<!--                    <input id="reply_content" name="reply_content" type="text" style="width: 95%" value="<?php /*echo esc_attr($item['reply_content'])*/?>"
                    ...
                    <textarea name="user_packet" type="text" id="user_packet"
                              cols="50" rows="6" value="<?php echo esc_attr($item['user_packet'])?>" required><?php echo esc_attr($item['user_packet'])?></textarea>
                </td>
            </tr>
            <tr class="form-field">
                <th valign="top" scope="row">
                    <label for="gender"><?php _e('gender', $this->plugin_slug)?></label>
                </th>
                <td>
...
<!--                           size="50" class="code" placeholder="--><?php //_e('reply type', $this->plugin_slug)?><!--" required>-->
            ...
            <tr class="form-field">
                ...
                <td>
                    <select name="status" id="status">
                        <option value="1" <?php selected( $selected, '1' ); ?>><?php _e("subscribe", $this->plugin_slug);?></option>
                        <option value="0" <?php selected( $selected, '0' ); ?>><?php _e("unsubscribe", $this->plugin_slug);?></option>
                    </select>
     ...
     * @return bool|string
     */
    function wechat_users_validate($item)
    {
        $messages = array();

        if (empty($item['openid'])) $messages[] = __('openid is required', $this->plugin_slug);
        if (empty($item['nickname'])) $messages[] = __('nickname is required', $this->plugin_slug);
        if (empty($item['city'])) $messages[] = __('city is required', $this->plugin_slug);
        if (empty($item['country'])) $messages[] = __('country is required', $this->plugin_slug);
        if (empty($item['province'])) $messages[] = __('province is required', $this->plugin_slug);
